...
const DeptCommissioner = () => {
    ...
    const handleDelete = async (id, type) => {
        try {
            if (type === "history") {
                await api.delete(`/dept-commissioner-desc/${id}`);
                setDescData((prevData) => prevData.filter((item) => item.id !== id));
            } else if (type === "co") {
                await api.delete(`/dept-commissioner-details/${id}`);
    ...
    const handleSaveChanges = async () => {
        try {
            if (modalType === "history") {
                ...
                await api.put(`/dept-commissioner-details/${selectedItem.id}`, formData, {
                    headers: {
                                ...
                                <div className="card-block">
                                    <div className="row ">
                                        ...
                                        <div className="col-sm-8 col-9 text-right m-b-20">
                                            <Link
                                                to={coData.length > 1 ? "#" : "/add-dept-commissioner-details"}
                                        ...
                                        <table className="table table-bordered m-b-0">
                                            ...
                                            <tbody>
                                                {currentPageData.length > 0 ? (
                                                    ...
                                                    <tr>
                                                        <td colSpan="3" className="text-center">No Deputy Commissioner Description Available</td>
                                                    </tr>
                                                )}
                                            </tbody>
                                        </table>
