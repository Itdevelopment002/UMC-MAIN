...
const AsstCommissioner = () => {
    ...
    const handleDelete = async (id, type) => {
        try {
            if (type === "history") {
                await api.post(`/delete-asst-commissioner-desc/${id}`);
                setDescData((prevData) => prevData.filter((item) => item.id !== id));
            } else if (type === "co") {
                await api.post(`/delete-asst-commissioner-details/${id}`);
    ...
    const handleSaveChanges = async () => {
        try {
            if (modalType === "history") {
                await api.post(`/edit-asst-commissioner-desc/${selectedItem.id}`, {
                ...
                await api.post(`/edit-asst-commissioner-details/${selectedItem.id}`, formData, {
                    headers: {
