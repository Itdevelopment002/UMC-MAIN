...
const AddOnlineHomeServices = () => {
  ...
  const validateForm = () => {
    ...
    if (!heading) {
      validationErrors.heading = "Service Name is required.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post("/online-services-home", {
        heading: heading,
        link: link,
      ...
      if (
        ...
        toast.error(
          error.response?.data?.message || "Failed to add online service. Try again.",
          {
            position: "top-right",
            autoClose: 3000,
          }
...
export default AddOnlineHomeServices;
