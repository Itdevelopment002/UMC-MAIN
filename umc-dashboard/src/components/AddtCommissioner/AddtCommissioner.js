...
const AddtCommissioner = () => {
    ...
    const fetchCoData = async () => {
        try {
            const response = await api.get("/addt-commissioner-data");
    ...
    const handleDelete = async (id) => {
        try {
            await api.post(`/delete-addt-commissioner-data/${id}`);
            setCoData((prevData) => prevData.filter((item) => item.id !== id));
            toast.success(
                `Additional Commissioner Data deleted successfully!`
            );
        } catch (error) {
            console.error(error);
    ...
    const handleSaveChanges = async () => {
        try {
            ...
            await api.post(`/edit-addt-commissioner-data/${selectedItem.id}`, formData, {
                headers: {
                            ...
                            <div className="card-block">
                                <div className="row">
                                    <div className="col-sm-4 col-3">
                                        <h4 className="page-title">Additional Commissioner Data</h4>
                                    </div>
                                    <div className="col-sm-8 col-9 text-right m-b-20">
                                        <Link to="/add-addt-commissioner-data" className="btn btn-primary btn-rounded float-right">
