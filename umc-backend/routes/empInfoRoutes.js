...
const { validateEmployeeInfo } = require("../middleware/validationinputfield.js");
...
router.post("/emp-info", verifyToken, sanitizeInput, validateEmployeeInfo, (req, res) => {
...
router.post("/edit-emp-info/:id", verifyToken, sanitizeInput, validateEmployeeInfo, (req, res) => {
  if (req.user?.role === "Admin") {
    return res.status(403).json({ message: "Permission denied: Admins are not allowed to perform this action." });
  }
  const { description, link, issue_date, language_code } = req.body;
