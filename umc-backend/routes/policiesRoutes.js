...
const { validatePolicies } = require("../middleware/validationinputfield.js");
...
router.post("/policies_data", verifyToken, sanitizeInput, validatePolicies, (req, res) => {
...
router.post("/edit-policies_data/:id", verifyToken, sanitizeInput, validatePolicies, (req, res) => {
  if (req.user?.role === "Admin") {
    return res.status(403).json({ message: "Permission denied: Admins are not allowed to perform this action." });
  }
  const { heading, link, issue_date, language_code } = req.body;
