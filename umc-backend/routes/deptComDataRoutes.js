...
router.post("/dept-commissioner-data", upload.single("coImage"), (req, res) => {
...
router.put("/dept-commissioner-data/:id", upload.single("coImage"), (req, res) => {
  const { id } = req.params;
  const { coName, designation, qualification, address, number, email, description, language_code } = req.body;
...
router.delete("/dept-commissioner-data/:id", (req, res) => {
  const { id } = req.params;
