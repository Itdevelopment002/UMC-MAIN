...
const { validatePropertyTaxDept } = require("../middleware/validationinputfield.js");
...
router.post("/property-dept", verifyToken, sanitizeInput, validatePropertyTaxDept, (req, res) => {
...
router.post("/edit-property-dept/:id", verifyToken, sanitizeInput, validatePropertyTaxDept, (req, res) => {
  if (req.user?.role === "Admin") {
    return res.status(403).json({ message: "Permission denied: Admins are not allowed to perform this action." });
  }
  const { description, link, issue_date, language_code } = req.body;
