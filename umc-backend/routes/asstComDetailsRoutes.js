...
router.post("/asst-commissioner-details", upload.single("coImage"), (req, res) => {
  ...
  db.query(sql, [coName, designation, qualification, address, number, email, language_code, imagePath], (err, result) => {
    ...
    res.status(200).json({ message: "Assistant Commissioner added successfully", id: result.insertId });
  });
});


router.put("/asst-commissioner-details/:id", upload.single("coImage"), (req, res) => {
  const { id } = req.params;
  const { coName, designation, qualification, address, number, email, language_code } = req.body;
  ...
  db.query(selectSql, [id], (err, result) => {
    ...
    db.query(updateSql, updateParams, (err, updateResult) => {
      ...
      res.status(200).json({ message: "Assistant Commissioner updated successfully" });
    });
  });
});


router.delete("/asst-commissioner-details/:id", (req, res) => {
  const { id } = req.params;
  ...
  db.query(selectSql, [id], (err, result) => {
    ...
    db.query(deleteSql, [id], (err, deleteResult) => {
      ...
      res.status(200).json({ message: "Assistant Commissioner deleted successfully" });
    });
  });
});
