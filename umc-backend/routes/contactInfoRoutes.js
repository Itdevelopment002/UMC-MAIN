...
const { validateFooterUpdateContact, validateFooterContact } = require("../middleware/validationinputfield.js");
...
router.post("/contacts-info", verifyToken, upload.single("image"), sanitizeInput, validateFooterContact, handleMulterError, (req, res) => {
...
router.post("/edit-contacts-info/:id", verifyToken, upload.single("image"), sanitizeInput, validateFooterUpdateContact, handleMulterError, (req, res) => {
  if (req.user?.role === "Admin") {
    return res.status(403).json({ message: "Permission denied: Admins are not allowed to perform this action." });
  }
  const { id } = req.params;
