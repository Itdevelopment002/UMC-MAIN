...
router.get("/online-services-home", (req, res) => {
  db.query("SELECT * FROM onlineservices", (err, results) => {
    if (err) throw err;
    res.json(results);
  });
});
...
router.post("/online-services-home", (req, res) => {
  ...
  const sql = "INSERT INTO onlineservices (heading, link) VALUES (?, ?)";
...
router.put("/online-services-home/:id", (req, res) => {
  const { heading, link } = req.body;
  const sql = "UPDATE onlineservices SET heading = ?, link = ? WHERE id = ?";
...
router.delete("/online-services-home/:id", (req, res) => {
  const sql = "DELETE FROM onlineservices WHERE id = ?";
