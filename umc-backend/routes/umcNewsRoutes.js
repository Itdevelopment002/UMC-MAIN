...
const { validateUMCNews } = require("../middleware/validationinputfield.js");
...
router.post("/umc-news", verifyToken, sanitizeInput, validateUMCNews, (req, res) => {
...
router.post("/edit-umc-news/:id", verifyToken, sanitizeInput, validateUMCNews, (req, res) => {
  if (req.user?.role === "Admin") {
    return res.status(403).json({ message: "Permission denied: Admins are not allowed to perform this action." });
  }
  const { heading, link, issue_date, language_code } = req.body;
...
router.post("/delete-umc-news/:id", verifyToken, (req, res) => {
  if (req.user?.role === "Admin") {
    return res.status(403).json({ message: "Permission denied: Admins are not allowed to perform this action." });
  }
