...
router.post("/proactive-disclosure", verifyToken, sanitizeInput, validateRti, (req, res) => {
...
router.post("/edit-proactive-disclosure/:id", verifyToken, sanitizeInput, validateRti, (req, res) => {
  if (req.user?.role === "Admin") {
    return res.status(403).json({ message: "Permission denied: Admins are not allowed to perform this action." });
  }
  const { description, link, issue_date, language_code } = req.body;
