...
const AsstCommissioner = () => {
  ...
  const fetchCommissionerData = async () => {
    try {
      const [detailsResponse, descResponse] = await Promise.all([
        api.get(`/asst-commissioner-details?lang=${i18n.language}`),
        api.get(`/asst-commissioner-desc?lang=${i18n.language}`)
      ]);
  ...
  const getDescriptionsForCommissioner = (commissionerName) => {
    return descriptions.filter(desc => desc.commissioner_name === commissionerName).map(item => item.description) || [];
  };

  const fetchHeaderImage = async () => {
    try {
      const response = await api.get("/banner");
      const selectedBanner = response.data.find(banner => banner.banner_name === "Assistant Commissioner");
      if (selectedBanner) {
      ...
      <div id="main-content">
        <div className="container-fluid font-location mt-2 mb-5" id="commissioner-css">
          ...
          <h2 className="location-title">
            ...
            <span className="highlighted-text"> {t('asst-commissioner.highlight-text')}</span>
          </h2>

          {commissioners.map((commissioner, index) => {
            const commissionerDescriptions = getDescriptionsForCommissioner(commissioner.coName);
            return (
                        ...
                        <div className="commissioner-col">
                          <div className="commissioner-item">
                            ...
                            <div className="text-box">
                              <strong className="label">{t('asst-commissioner.name')} :</strong>
...
//     try {
//       const response = await api.get(`/asst-commissioner-details?lang=${i18n.language}`);
...
//     try {
//       const response = await api.get(`/asst-commissioner-desc?lang=${i18n.language}`);
...
//         let selectedBanner = response.data.find(banner => banner.banner_name === "Assistant Commissioner");
